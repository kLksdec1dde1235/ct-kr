$(document).ready(function(){
    $('#send_message').click(function(){
        ...
        // var agree = $('#agree11').is(":checked");
         ...
         $('#form_e11').prop("action", "https://docs.google.com/forms/u/0/d/e/1FAIpQLSd1w3tFofaJRjgAMcdPDkUJMibrynrRLg3NIZNRJb8Gxhul1g/formResponse");
...
function form_c() {
  ...
  if (!position1) return disableButton("소속기관 선택을 확인하세요");
  if (!position2) return disableButton("담당직무 선택을 확인하세요");
  if (!position)  return disableButton("서비스 품목 선택을 확인하세요");
  if (!position3) return disableButton("서비스 기간 선택을 확인하세요");
  if (!position4) return disableButton("렌탈 경험 유무 선택을 확인하세요");
  // if (!position5) return disableButton("서비스 선택을 확인하세요");
